...
def check_cluster_status(cluster_id, node_ids):
    ...
    check_active_status = lambda x: ok_status if(x == "OK") else not_ok_status

    ok_status = {'cluster_id': cluster_id,
                 'cluster_values': {'status': models.Status.ACTIVE}}
    not_ok_status = {'cluster_id': cluster_id,
    ...
    check_cluster_status = os_common.Lambda(
        check_active_status,
        name="translate status to active or error",
        rebind={'cluster_status': "cluster_status"},
        provides="final_cluster_status"
    )
    flow.add(check_cluster_status)

    update_cluster_status = cue_tasks.UpdateClusterStatus(
        name="update cluster status %s" % cluster_id,
        rebind={'status': "final_cluster_status"},
        requires="status")
