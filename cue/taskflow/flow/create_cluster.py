...
def create_cluster(context, cluster_id, node_ids):
    """Create Cluster flow factory function

    This factory function uses :func:`cue.taskflow.flow.create_cluster_node` to
    create a multi node cluster.

    :param project_id: Unique ID for the project/tenant the cluster is part of.
    ...
    start_flow_status = {'context': context,
                         'cluster_id': cluster_id,
                         'cluster_status': 'BUILDING'}
    end_flow_status = {'context': context,
                       'cluster_id': cluster_id,
                       'cluster_status': 'ACTIVE'}

    for i, node_id in enumerate(node_ids):
        sub_flow.add(create_cluster_node.create_cluster_node(context,
                                                             cluster_id, i,
                                                             node_id))
