...
import cue.taskflow.os_utils as os_utils
...
def create_cluster():
    #vm_status_predicate = lambda vm_status: vm_status == 'vm_status'
    flow = linear_flow.Flow('creating vm').add(
        neutron_task.CreatePort(os_client=os_utils.neutron_client(),
                                provides='neutron_port_id'),
        cinder_task.CreateVolume(os_client=os_utils.cinder_client(),
                                 provides='cinder_volume_id'),
        nova_task.CreateVm(os_client=os_utils.nova_client(),
                           provides='vm_id'),
        linear_flow.Flow('wait for vm to become active',
                         retry=retry.Times(10)).add(
            nova_task.GetVmStatus(os_client=os_utils.nova_client(),
                                  provides='vm_status'),
            common_task.CheckFor(rebind={'check_var': 'vm_status'},
                                 check_value='ACTIVE',
                                 timeout_seconds=1),
            #common_task.Assert(predicate=vm_status_predicte,
            #                   timeout_ms=1000)
        ),
