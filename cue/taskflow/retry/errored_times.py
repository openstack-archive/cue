...
class ExceptionTimes(retry.Times):
    ...
    def __init__(self, revert_exception_list=[], attempts=1, name=None,
                 provides=None, requires=None, auto_extract=True, rebind=None,
                 revert_all=False):
    ...
    def on_failure(self, history, *args, **kwargs):
        ...
        for owner, outcome in failures:
            error_details.append(outcome)

        error = error_details[len(error_details) - 1]

        if type(error.exception) in self._revert_exception_list:
