...
class ClusterObjectsTests(base.FunctionalTestCase):
    ...
    def test_cluster_object_generation(self):
        ...
        test_utils.validate_cluster_values(self, cluster_object, cluster_dict)
    ...
    def test_create_cluster(self):
        """Tests create a cluster from Cluster objects API."""
        ...
        for nodes in cluster_nodes:
            self.assertEqual(nodes.cluster_id, db_cluster.id)
    ...
    def test_update(self):
        ...
        new_cluster.name = 'test_cluster'
        new_cluster.update(self.context, new_cluster.id)

        test_cluster = self.dbapi.get_cluster_by_id(self.context,
                                                    new_cluster.id)
        self.assertEqual('flavor2', test_cluster.flavor)
        self.assertEqual('test_cluster', test_cluster.name)

    def test_get_clusters(self):
        """Tests getting all Clusters from Cluster objects API."""
        new_cluster = func_utils.create_object_cluster(self.context)
        cluster_list = new_cluster.get_clusters(self.context)
    ...
    def test_get_cluster_by_invalid_id(self):
        """Tests get cluster from Cluster objects API with invalid cluster id.
