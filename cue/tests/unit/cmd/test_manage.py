...
from cue.manage import broker as cue_broker
from cue.manage import database as cue_db
from cue.manage import taskflow as cue_taskflow
...
class TestManage(base.UnitTestCase):
    ...
    @mock.patch('cue.objects.broker_metadata.BrokerMetadata'
                '.get_broker_metadata_by_broker_id')
    def test_fetch_func_args_broker(self, mock_list_metadata):
        """Asserts the cue.cmd.manage.fetch_func_args() returns the arguments
        ...
        category_opt = cfg.SubCommandOpt('category', title='Commands',
                                         ...
                                         handler=manage.add_command_parsers)
        cfg.CONF.register_cli_opt(category_opt)
        sys_argv = ["cue-manage", "broker", "list_metadata", "fake_broker_id"]
        cfg.CONF(sys_argv[1:], project='cue',
             version=version.version_info.version_string())

        cue_broker_commands = cue_broker.BrokerCommands()
        args = manage.fetch_func_args(cue_broker_commands.list_metadata)
        self.assertEqual(args, ['fake_broker_id'])

    def test_fetch_func_args_taskflow(self):
        """Asserts the cue.cmd.manage.fetch_func_args() returns the arguments
        ...
        category_opt = cfg.SubCommandOpt('category', title='Commands',
                                         ...
                                         handler=manage.add_command_parsers)
        cfg.CONF.register_cli_opt(category_opt)
        sys_argv = ["cue-manage", "taskflow", "upgrade"]
        cfg.CONF(sys_argv[1:], project='cue',
             version=version.version_info.version_string())

        cue_taskflow_commands = cue_taskflow.TaskFlowCommands()
        args = manage.fetch_func_args(cue_taskflow_commands.upgrade)
