...
class CreateVmTests(base.TestCase):
    ...
    def setUp(self):
        ...
        if len(CreateVmTests.additional_fixtures) > 0:
            valid_image_name = "cirros-0.3.2-x86_64-uec-kernel"
            valid_flavor_name = "m1.tiny"
            valid_network_name = "test-network"
        else:
            valid_image_name = "Ubuntu Server 14.10 (amd64 20141022.3) - Partner Image"
            valid_flavor_name = "standard.small"
            valid_network_name = "min.pae@hp.com-network"
        ...
        self.new_vm_id = None
        self.valid_image = self.nova_client.images.find(name=valid_image_name)
        self.valid_flavor = self.nova_client.flavors.find(name=valid_flavor_name)

        network_list = self.neutron_client.list_networks(name=valid_network_name)
    ...
    def test_create_vm_invalid_flavor(self):
        ...
        self.assertRaises(nova_exc.BadRequest, engines.run, self.flow, store=flow_store)
    ...
    def test_create_vm_missing_attributes(self):
        ...
        self.assertRaises(taskflow_exc.MissingDependencies, engines.run, self.flow, store=flow_store)
