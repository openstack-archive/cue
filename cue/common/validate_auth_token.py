...
class AuthTokenValidator(object):
    ...
    def validate_token(auth_type, broker_type, token):
        ...
        if auth_type.upper() == AuthTokenValidator.PLAIN_AUTH:
            if broker_type.upper() == AuthTokenValidator.RABBIT_BROKER:
                auth_validator = RabbitPlainAuthTokenValidator(token=token)
            else:
                LOG.error(_LE('Invalid broker type received: %(broker_type)')
                          % ({"broker_type": broker_type}))
        else:
            LOG.error(_LE('Invalid authentication type received: %(auth_type)')
                      % ({"auth_type": auth_type}))
        ...
        if self.token is not None:
            if 'username' in self.token:
                if ((len(self.token['username']) >=
                    ...
                    LOG.error(_LE('Invalid username: %(username)') % (
                        {"username": self.token['username']}))
                    valid_username = True
            if 'password' in self.token:
                if ((len(self.token['password']) >=
                    ...
                    LOG.error(_LE('Invalid username: %(password)') % (
