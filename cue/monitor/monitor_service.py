...
class MonitorService(service.Service):
    ...
    def check(self):
        ...
        if self.lock.acquired:
            ...
            job_list = filter(lambda job: 'cluster_status_check' in job.details['store'], job_list)
            job_list = map(lambda job: job.details['store']['cluster_id'], job_list)
            clusters = filter(lambda cluster: cluster[0] not in job_list, clusters)
